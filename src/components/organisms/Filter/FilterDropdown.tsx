...
export default function FilterDropdown(props: FilterDropdownProps) {
  const { label, options: rawOptions, value: rawValue, onChange, showAllOption = true, multiple = false } = props;
  ...
  const displayText = value.length === 0 || (value.length === 1 && value[0] === allLabel)
    ? allLabel
    : value.length === 1
  ...
  return (
    <Listbox value={multiple ? value : value[0]} onChange={handleChange} multiple={multiple}>
      <div className="relative w-full">
        <ListboxButton className="w-full appearance-none border-y-4 border-t-transparent border-b-neutral-300 bg-neutral-200 px-3 py-2 pr-10 text-neutral-700 cursor-pointer focus:outline-none focus:border-b-primary-600 text-left">
          ...
          <span className="pointer-events-none absolute inset-y-0 right-0 flex items-center pr-3">
            <ChevronDownIcon class="h-5 w-5 text-neutral-500" aria-hidden="true" />
          </span>
        </ListboxButton>
        
        <ListboxOptions className="absolute z-10 mt-1 max-h-60 w-full overflow-auto bg-white py-1 text-base shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none">
          {options.map((option) => (
            <ListboxOption
              key={option}
              value={option}
