...
[NDT Unified Views](/tests/ndt/#unified-views) and the [statistics pipeline](https://github.com/m-lab/stats-pipeline/#statistics-pipeline-service) are optimized to provide
...
Unified views are are built on [extended views](/tests/ndt/#extended-views), which are a
...
The Unified Views are a `UNION` of selected fields from all three NDT data types, with a filter applied to only present completed, valid test results [according to our current, best understanding](/tests/ndt/#unified-views).
