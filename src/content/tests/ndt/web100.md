...
web100 was the NDT protocol used by M-Lab until November 2019. As a part of M-Lab's platform upgrade, [ndt-server](https://github.com/m-lab/ndt-server/) replaced the now deprecated [web100-based NDT server](https://github.com/m-lab/ndt/). The new server provides the [ndt5 protocol](/tests/ndt/ndt5) for backward compatibility with current clients, and will add the ndt7 protocol for future client use.

NDT data using the web100 protocol was collected using the [Web100 Linux kernel](https://dl.acm.org/citation.cfm?id=956993.957002) patch to provide access to a rich set of TCP information for each NDT test.

web100 NDT data can be found in the [NDT web100 dataset](https://console.cloud.google.com/bigquery?project=measurement-lab&folder&organizationId=433637338589&p=measurement-lab&d=ndt&t=web100&page=table).

## Web100 Field Types and BigQuery Equivalents

Web100 fields were defined by the original working group in the file [tcp-kis.txt](https://cloud.google.com/bigquery/docs/tcp-kis.txt). This file defines each Web100 variable with a specific [SNMP type](http://tools.ietf.org/html/rfc4898). The table below shows how to map each SNMP type to a BigQuery type.
...
<!-- TODO: inline schema_web100_faithful.html -->
