...
To illustrate the use of _ndt-server_ outside of the M-Lab context, I put together this post as a demo so others might take _ndt-server_ for a test drive in their networks. I've used a publicly available [ndt fullstack](https://github.com/m-lab/ndt-server/tree/master/fullstack) Docker image for NDT from [M-Lab's Dockerhub](https://hub.docker.com/r/measurementlab/ndt). The fullstack image provides the \_ndt-server_ container, and all of the "sidecar" container services that M-Lab uses on our servers to gather rich data from client tests: _tcp-info_, _traceroute_, _uuid_, and _packet header captures_.
