---
permalink: run-your-own-ndt-server
title: Running Your Own ndt-server
excerpt: >-
  If you've followed the M-Lab blog over the past year and a half, you'll know
  that we've been engineering and testing a massive upgrade to our server
  software and how it's managed and deployed \[1\] \[2\] \[3\] \[4\]. That's
...
To illustrate the use of _ndt-server_ outside of the M-Lab context, I put together this post as a demo so others might take _ndt-server_ for a test drive in their networks. I've used a publicly available [ndt fullstack](https://github.com/m-lab/ndt-server/tree/master/fullstack) Docker image for NDT from [M-Lab's Dockerhub](https://hub.docker.com/repository/docker/measurementlab/ndt). The fullstack image provides the \_ndt-server_ container, and all of the "sidecar" container services that M-Lab uses on our servers to gather rich data from client tests: _tcp-info_, _traceroute_, _uuid_, and _packet header captures_.
...
Fortunately, all this is possible and then some. In my next demo post, I'll demo how to use two of our supported command line client libraries, [_libndt_](https://github.com/measurement-kit/libndt/) and [\_ndt7-client-go_](https://github.com/m-lab/ndt7-client-go), and also demo _[Murakami](https://github.com/m-lab/murakami)_ an automated test runner container with some really great features. Stay tuned!
